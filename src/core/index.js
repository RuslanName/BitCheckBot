...
const { PORT, WEBHOOK_DOMAIN, BOT_MODE } = require('../config');
...
const { authRoutes, configRoutes, userRoutes, dealRoutes, broadcastRoutes, raffleRoutes, withdrawalRoutes, analyticsRoutes, cashbackRoutes } = require('../routes');

const setupBroadcastRoutes = (router, emitter) => {
    router.use((req, res, next) => {
        req.broadcastEmitter = emitter;
        const mainBotModule = require('../../main-bot');
        req.main_bot = mainBotModule.getBot();
        const { telegramWithRetry } = require('../utils/retry-utils');
        req.telegramWithRetry = telegramWithRetry;
        next();
    });
    return router;
};
...
app.get('/cashback', (req, res) => {
    sendHtmlFile(res, path.join(__dirname, '../../public/html/cashback.html'));
});
...
const initBot = async () => {
    ...
    if (BOT_MODE === 'webhook') {
        ...
        try {
            await mainBot.telegram.deleteWebhook();
            await mainBot.launch();
            console.log('Bot started in polling mode');
        } catch (error) {
            console.error('Error starting polling bot:', error.message);
        }
    }
};
