...
function calculateUserStats(userId) {
    ...
    const stats = {
        ...
        boughtXMR: { rub: 0, crypto: 0 },
        soldBTC: { rub: 0, crypto: 0 },
        soldLTC: { rub: 0, crypto: 0 },
        soldXMR: { rub: 0, crypto: 0 }
    };
    ...
    userDeals.forEach(deal => {
        if (deal.type === 'buy') {
            ...
            } else if (deal.currency === 'XMR') {
                stats.boughtXMR.rub += deal.rubAmount || 0;
                stats.boughtXMR.crypto += deal.cryptoAmount || 0;
            }
            ...
            if (deal.currency === 'BTC') {
                ...
                stats.soldXMR.rub += deal.rubAmount || 0;
                stats.soldXMR.crypto += deal.cryptoAmount || 0;
            }
        }
    });
...
async function checkIfBlocked(ctx) {
    ...
    if (user && user.isBlocked) {
        const { sendBitCheckPhoto } = require('../utils/telegram-utils');
